"""
This file is the template for the boilerplate of train/test of a DNN for image segmentation
...
import models
# Delegated
from template.runner.image_segmentation import evaluate, train
...
class ImageSegmentation:
    ...
    def single_run(writer, current_log_folder, model_name, epochs, lr, decay_lr,
        ...
        ImageSegmentation._validate_model_input_size(model_expected_input_size, model_name)
        ...
        val_value[-1] = ImageSegmentation._validate(val_loader, model, criterion, writer, -1, **kwargs)
        for epoch in range(start_epoch, epochs):
            # Train
            train_value[epoch] = ImageSegmentation._train(train_loader, model, criterion, optimizer, writer, epoch,
                                                            **kwargs)

            # Validate
            if epoch % validation_interval == 0:
                val_value[epoch] = ImageSegmentation._validate(val_loader, model, criterion, writer, epoch, **kwargs)
        ...
        # Test
        test_value = ImageSegmentation._test(test_loader, model, criterion, writer, epochs - 1, **kwargs)
