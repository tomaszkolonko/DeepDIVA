...
def _evaluate(data_loader, model, criterion, writer, epoch, logging_label, no_cuda=False, log_interval=10, **kwargs):
    ...
    # Generate a segmentation report for each epoch
    _log_segmentation_report(data_loader, epoch, preds, targets, writer)

    return top1.avg


def _log_segmentation_report(data_loader, epoch, preds, targets, writer):
    """
    This routine computes and prints on Tensorboard TEXT a segmentation
    ...
    with warnings.catch_warnings():
        ...
        segmentation_report_string = str(classification_report(y_true=targets,
                                                                 y_pred=preds,
    ...
    segmentation_report_string = segmentation_report_string.replace('\n ', '\n\n       ')
    segmentation_report_string = segmentation_report_string.replace('precision', '      precision', 1)
    segmentation_report_string = segmentation_report_string.replace('avg', '      avg', 1)

    writer.add_text('Segmentation Report for epoch {}\n'.format(epoch), '\n' + segmentation_report_string, epoch)
